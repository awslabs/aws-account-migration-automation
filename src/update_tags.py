...
def get_data(s3_client, bucket_name: str, object_key: str):
    ...
    if not re.search(r'^\S+.json$', object_key):
        raise Exception("File format not supported, Only '.json' format is supported as of now.")

    try:
        return s3_client.get_object(Bucket=bucket_name, Key=object_key)

    except ClientError as ce:
        if ce.response['Error']['Code'] == 'AccessDenied' or ce.response['Error']['Code'] == 'NoSuchKey':
            return {}
        raise ce
