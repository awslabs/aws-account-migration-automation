...
try:
    conn = pymysql.connect(
        ...
        user=os.environ["databaseUsername"],
        passwd=os.environ["databaseUserpass"],
        db=os.environ["databaseName"],
        connect_timeout=10,
    )
except:
...
def get_secret_hash(username):
    ...
    dig = hmac.new(
        str("1m1sbuqiippjg1smai113ihvebtu9si9gm6m14nvs6fckn2rhp9k").encode("utf-8"),
...
def lambda_handler(event, context):
    try:
        ...
        if "email" in event and event["email"] != "":
            if (
                checkEmailPhoneExistsSameGroup(
                    event["group"], {"email": event["email"]}
                )
                == 0
            ):
        ...
        if "mobile" in event and event["mobile"] != "":
            if (
                checkEmailPhoneExistsSameGroup(
                    event["group"], {"mobile": event["mobile"]}
                )
                == 0
            ):
                UserAttributes.append(
                    {"Name": "phone_number", "Value": event["mobile"]}
                )
                UserAttributes.append(
        ...
        try:
            if Username != "" and "mobile" in event and event["mobile"] != "":
                ...
                try:
                    ...
                    if mobile != None:
                        text_massage = (
                            "DrKure: Thank you for joining us. It is recommended to change your temporary password immediately. Your username is "
                            + Username
                            + " and temporary password is "
                            + tempPass
                        )
                    ...
                    inputdata.update(
                        {"created": MyHelper.timeToStore("", event["time_zone"])}
                    )
                    inputdata.update(
                        {"modified": MyHelper.timeToStore("", event["time_zone"])}
                    )
                    ...
                    if "diagnostic_admin" in event.keys():
                        inputdata.update(
                            {"diagnostic_admin": event["diagnostic_admin"]}
                        )
                    ...
                    if "hospital_doctor_admin" in event.keys():
                        inputdata.update(
                            {"hospital_doctor_admin": event["hospital_doctor_admin"]}
                        )
                    ...
                    try:
                        with conn.cursor(pymysql.cursors.DictCursor) as cur:
                            cur.execute(
                                MyHelper.queryInsert(
                                    MyHelper.getTableFromGroup(event["group"]),
                                    inputdata,
                                )
                            )
                            conn.commit()
                            if "language" in event and event["language"] != "":
                                ...
                                for l in language:
                                    ...
                                    conn.commit()
                            if (
                                "specialization" in event
                                and event["specialization"] != ""
                                and event["group"] == "Doctor"
                            ):
                            ...
                            response = MyHelper.getSelectTable(
                                "GetUser",
                                event["time_zone"],
                                event["group"],
                                awsUsername,
                            ...
                            return {
                                ...
                                "data": json.dumps(
                                    response, default=MyHelper.myConverter
                                ),
                            }
                    except:
                        userdelete = client.admin_delete_user(
                            UserPoolId=UserPoolId, Username=awsUsername
                        )
                        return {
                            "status": "error",
