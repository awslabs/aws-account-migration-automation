    ...
    try:
        ...
        if account["AccountType"] == Constant.AccountType.LINKED:
            ...
            master_session = get_session(
                f"arn:aws:iam::{master_account_id}:role/{master_record['AdminRole']}"
            )
            organization_client = master_session.client("organizations")

            account_info = organization_client.describe_account(
            ...
            if Constant.ACCOUNT_EMAIL_VALIDATION.__eq__(Constant.TRUE) and not bool(
                ...
                account["Error"] = log_error(
                    ...
                    error_type=Constant.ErrorType.CATE,
                    msg=msg,
                    notify=True,
        ...
        # Master account Flow
            ...
            master_session = get_session(
                f"arn:aws:iam::{account['AccountId']}:role/{account['AdminRole']}"
            )
            organization_client = master_session.client("organizations")
        ...
        # Standalone account Flow
            ...
            try:
                ...
                raise Exception(
                    f"Standalone account {account['AccountId']} has organization setup."
                )
        ...
        account["AccountStatus"] = (
            Constant.AccountStatus.INVITED
            if account["Migrate"]
