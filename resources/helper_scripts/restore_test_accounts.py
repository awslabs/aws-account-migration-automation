...
MASTER_ACCOUNT = Constant.MASTER_ACCOUNT_ID
MASTER_ROLE = Constant.AWS_MASTER_ROLE
DYNAMODB_TABLE_NAME = Constant.DB_TABLE
_SHARED_RESOURCE_BUCKET = Constant.SHARED_RESOURCE_BUCKET
...
def restore_test_env(company_name, xlspath):
    ...
    try:
        ...
        try:
            account_data = get_accounts_by_company_name(
                table=DYNAMODB_TABLE_NAME, company_name=company_name)
        ...
        master_session = get_session(
            f"arn:aws:iam::{MASTER_ACCOUNT}:role/{MASTER_ROLE}")
        ...
        if test_master_account_id:
            ...
            try:
                test_master_session = get_session(
                    f"arn:aws:iam::{test_master_account_id}:role/{MASTER_ROLE}")
                test_organization_client = test_master_session.client(
                    'organizations')
                if test_organization_client.describe_organization()['Organization']['MasterAccountId'] != \
                    ...
                    organization_client.remove_account_from_organization(
                        AccountId=test_master_account_id)
                ...
                logger.info(
                    f"Creating organization for test master account with accountid as {test_master_account_id}")
        ...
        for account in account_data:
            ...
            account_session = get_session(
                f"arn:aws:iam::{account['AccountId']}:role/{MASTER_ROLE}")
            ...
            handshake_id = get_invitation(
                test_organization_client, account.get('AccountId'))
            try:
                if account['AccountType'] == Constant.AccountType.LINKED:
                    ...
                    if not handshake_id:
                        response = test_organization_client.invite_account_to_organization(
                            Target={
                                'Id': account['AccountId'], 'Type': 'ACCOUNT'},
                    ...
                    linked_org_client.accept_handshake(
                        HandshakeId=handshake_id)
                    logger.info(
                        f"Invitation {handshake_id} is being accepted by the test account successfully.")
            except Exception as ex:
                logger.error(ex)
                pass
            ...
            roles_to_deleted = set(Constant.ROLE_CONFIG.keys()).difference(
                set([account['AdminRole']]))
            for role in roles_to_deleted:
                try:
                    role_obj = iam.Role(role)
                    policy_arns = [
                        policy.arn for policy in role_obj.attached_policies.all()]
                    ...
                    role_obj.delete()
                    logger.info(
                        f"Role {role} is success fully deleted for account id {account['AccountId']}")
                except ClientError as ce:
                    logger.info(ce)
...
if __name__ == '__main__':
    restore_test_env("Xyz", "AWS_Test.xls")
